...
function weatherCity(t){
  ...
  if(raw.includes("conil")||raw.includes("cádiz")||raw.includes("cadiz")) return {name:"Conil", lat:36.277, lon:-6.088};
  if(raw.includes("sevilla")) return {name:"Sevilla", lat:37.389, lon:-5.984};
  if(raw.includes("bruselas")||raw.includes("brussels")) return {name:"Bruselas", lat:50.846, lon:4.352};
  if(raw.includes("londres")||raw.includes("london")) return {name:"Londres", lat:51.507, lon:-0.128};
  if(raw.includes("roma")) return {name:"Roma", lat:41.902, lon:12.496};
  return null;
}
...
function weatherText(code){
  if(code===0)return"Soleado"; if([1,2].includes(code))return"Parcialmente nublado"; if(code===3)return"Nublado";
  if([45,48].includes(code))return"Niebla"; if([51,53,55,56,57].includes(code))return"Llovizna";
  if([61,63,65,66,67,80,81,82].includes(code))return"Lluvia"; if([71,73,75,77,85,86].includes(code))return"Nieve";
  if([95,96,99].includes(code))return"Tormenta"; return"Tiempo";
}
async function fetchWeather(t){
  let c=weatherCity(t); if(!c)return null;
  let key="weather_"+c.name;
  try{let cached=JSON.parse(sessionStorage.getItem(key)||"null"); if(cached && Date.now()-cached.time<30*60*1000)return cached.data;}catch(e){}
  try{
    let url=`https://api.open-meteo.com/v1/forecast?latitude=${c.lat}&longitude=${c.lon}&current=temperature_2m,weather_code&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,weather_code&timezone=auto&forecast_days=3`;
    let r=await fetch(url); let j=await r.json();
    let data={city:c.name,temp:Math.round(j.current.temperature_2m),code:j.current.weather_code,days:j.daily};
    sessionStorage.setItem(key,JSON.stringify({time:Date.now(),data}));
    return data;
  }catch(e){return null}
}
async function updateWeatherCards(){
  let cards=document.querySelectorAll("[data-weather-trip]");
  for(let el of cards){
    let t=db.trips.find(x=>x.id===el.dataset.weatherTrip);
    let w=await fetchWeather(t);
    if(!w){el.innerHTML="<span>—</span>Tiempo";continue;}
    el.innerHTML=`<span>${weatherIcon(w.code)} ${w.temp}°</span>${w.city}<br><small>${weatherText(w.code)}</small>`; el.classList.add("weather-click"); el.onclick=()=>showWeatherDetail(t,w);
  }
}
function showWeatherDetail(t,w){
  if(!w)return;
  let days=w.days||{};
  let html=`<h3>${weatherIcon(w.code)} ${w.city}</h3><p><b>${w.temp}°</b> · ${weatherText(w.code)}</p><div class=weatherDetail>`;
  for(let i=0;i<Math.min(3,(days.time||[]).length);i++){
    let label=i===0?"Hoy":(i===1?"Mañana":"Pasado mañana");
    let code=days.weather_code?.[i];
    let max=Math.round(days.temperature_2m_max?.[i]);
    let min=Math.round(days.temperature_2m_min?.[i]);
    let rain=days.precipitation_probability_max?.[i];
    html+=`<div class=weatherDay><b>${label}</b><span>${weatherIcon(code)} ${weatherText(code)}</span><span>Máx ${max}° · Mín ${min}° · Lluvia ${rain ?? 0}%</span></div>`;
  }
  html+=`</div><p class=muted>Se actualiza automáticamente al abrir la app.</p>`;
  showModal("Tiempo del destino",html);
}
...
function home(){
  let next=db.trips[0];
  app.innerHTML=`${next?homeHero(next):`<section class=hero><div class=txt><h1>Viajes & Plans 14 Stable</h1><p>V14 Stable Final</p></div></section>`}
...
function linkify(txt){
  ...
  txt = txt.replace(/^\s*[•\-]?\s*📍\s*(.{2,120})$/gmi,(m,place)=>{ if(/PUNTOS CLAVE|MAPS/i.test(place)) return m;
    let p=place.trim();
  ...
  txt = txt.replace(/^(.{2,140}?)\s+Maps\s*$/gmi,(m,place)=>/PUNTOS CLAVE|ENLACES|OPCIONES/i.test(place)?m:`${place} <a class=mapBtn target=_blank href="${googleMapsSearch(place)}">Abrir Maps</a>`);
  return txt;
}

function shouldAutoMapLine(line){
  let p=String(line||"").trim();
  if(!p)return false;
  if(p.includes("<a ")||p.includes("http")||p.includes("Abrir Maps"))return false;
  if(/^(Mañana|Tarde|Noche|Tips?|TIPS PRO|Resumen|Cronograma|Logística|Enlaces Útiles|Puntos Clave|PUNTOS CLAVE|Opciones|OPCIONES|Recomendación|Dirección|Todo a pie)/i.test(p))return false;
  if(/^[•\-\s]*\d{1,2}[:.]\d{2}/.test(p))return false;
  if(/^[0-9:–\-| ]+$/.test(p))return false;
  // numbered place lines: "1. Peck 47 (Desayuno)"
  if(/^\d+\.\s*.{2,90}$/.test(p))return true;
  // pin lines
  if(/^📍\s*.{2,90}$/.test(p))return true;
  // common proper place patterns
  if(/\b(Peck|Grand Place|Manneken|Galeries|Catedral|Frites|Nuetnigenough|Sablon|Palacio|Mont des Arts|Maison|Moeder|Noordzee|Bruxelles|Central|Aeropuerto|Dandoy|Lambic|Siècle|Leon|Léon)\b/i.test(p))return true;
  return false;
}
function placeText(line){
  return String(line||"")
    .replace(/^\s*[•\-]?\s*/,"")
    .replace(/^\d+\.\s*/,"")
    .replace(/^📍\s*/,"")
...
function formatNoteText(rawHtml){
  let text=rawHtml;
  // first keep real URLs and Apple Notes Maps patterns from linkify
  text=linkify(text);

  // Now add individual Maps buttons to list/place lines.
  let lines=text.split("\\n");
  let out=[];
  for(let line of lines){
    let plain=line.replace(/<[^>]+>/g,"").trim();
    if(shouldAutoMapLine(line)){
      let p=placeText(plain);
      out.push(`${line}<br><a class="autoMap" target="_blank" href="${googleMapsSearch(p)}">Abrir Maps</a>`);
    }else{
      out.push(line);
    }
  }
...
function tripForm(id=""){let t=id?db.trips.find(x=>x.id===id):{};showModal(id?"Editar viaje":"Nuevo viaje",`<input id=f1 placeholder=Nombre value="${esc(t.name||"")}"><input id=f2 placeholder=Fechas value="${esc(t.dates||"")}"><input id=f3 placeholder=Destino value="${esc(t.destination||"")}"><input id=f4 type=date placeholder="Fecha inicio" value="${esc(t.startDate||"")}"><textarea id=f5 placeholder=Resumen>${esc(t.summary||"")}</textarea><input id=f6 placeholder="URL imagen portada" value="${esc(t.cover||"")}"><input id=f7 placeholder="Hotel principal" value="${esc(t.hotel||"")}"><input id=f8 placeholder="Dirección/Maps hotel" value="${esc(t.hotelMap||"")}"><input id=f9 placeholder="Ciudad para el tiempo (ej: Conil, Bruselas)" value="${esc(t.weatherCity||"")}"><input id=f10 placeholder="Enlace carpeta iCloud/Drive del viaje" value="${esc(t.folderLink||"")}"><button class=primary onclick="saveTrip('${id}')">Guardar</button>`)}
function saveTrip(id=""){let t=id?db.trips.find(x=>x.id===id):{id:uid(),notes:[]};Object.assign(t,{name:f1.value,dates:f2.value,destination:f3.value,startDate:f4.value,summary:f5.value,cover:f6.value,hotel:f7.value,hotelMap:f8.value,weatherCity:f9.value,folderLink:f10.value});if(!id)db.trips.push(t);save();modal.classList.remove("show");view="trips";render()}
...
function shareTrip(id){
  let t=db.trips.find(x=>x.id===id); if(!t)return;
