...
async function updateWeatherCards(){
  ...
  for(let el of cards){
    ...
    el.innerHTML=`<span>${weatherIcon(w.code)} ${w.temp}°</span>${w.city}<br><small>${weatherText(w.code)}</small>`;
  }
}
...
function home(){
  let next=db.trips[0];
  app.innerHTML=`${next?homeHero(next):`<section class=hero><div class=txt><h1>Viajes & Plans 10</h1><p>Versión final</p></div></section>`}
  ...
  <div class=card><h2>Próximos viajes</h2>${db.trips.map(t=>`<div class="visualCard" onclick="activeTripId='${t.id}';view='trips';render()">${t.cover?`<img src="${esc(t.cover)}">`:""}<div class=vtxt><h2>${esc(t.name)}</h2><p>${esc(t.dates)} ${countdown(t.startDate||t.dates)?'· '+countdown(t.startDate||t.dates):''}</p></div></div>`).join("")||"<p class=muted>Sin viajes.</p>"}</div>
  <div class=smallGrid><button class=primary onclick=tripForm()>+ Viaje</button><button class=primary onclick=eventForm()>+ Evento</button><button class=primary onclick=reservationForm()>+ Reserva</button><button class=primary onclick=docForm()>+ Documento</button></div>`;
...
function autoMaps(txt){
  return txt.replace(/^(\s*(?:\d+\.\s*)?[A-ZÁÉÍÓÚÑa-záéíóúñ][^<\n]{2,80})$/gmi,(m,place)=>{
    let p=place.trim();
    if(!p || /^(Día|Fecha|Resumen|Notas|Tip|Recomendación|Transporte|Cena|Comida|Desayuno|Paseo|Llegada|Salida|Hotel|Alojamiento)$/i.test(p)) return m;
    if(/^[0-9:– -]+$/.test(p)) return m;
    if(p.includes("http")||p.includes("Abrir")) return m;
    return `${p} <a class=autoMap target=_blank href="${googleMapsSearch(p)}">Maps</a>`;
  });
}
...
function shareTrip(id){
  ...
  showModal("Compartir viaje",`<p class=muted>Copia este texto para WhatsApp, Notas o Mail.</p><div class=shareBox>${esc(text)}</div><button class=primary onclick="navigator.clipboard.writeText(\\`${text.replace(/`/g,"'")}\\`)">Copiar</button>`);
}
